  ...
  return (
    <div className="min-h-screen w-screen grid grid-cols-1 lg:grid-cols-2 overflow-hidden">
        ...
        >
          <CarouselContent>
            {sliderImages.map((src, index) => (
              <CarouselItem key={index} className="relative w-full h-[calc(100vh-2rem)]">
                <Image
                ...
                <div className="absolute bottom-28 left-12 text-white max-w-lg z-10">
                  <h3 className="text-3xl font-extrabold leading-snug">Seamless & Secure Access to Your Therapy EMR</h3>
                  <p className="mt-4 text-lg font-medium leading-relaxed">
                    Effortless login with multi-factor authentication, ensuring secure access to patient therapy
                    evaluations anytime.
                  </p>
                </div>
              </CarouselItem>
      ...
      <div className="relative flex flex-col justify-center px-6 sm:px-12 md:px-20 lg:px-24 py-10 sm:py-14 md:py-16">
        ...
        {/* Form */}
        <form
          className="space-y-5 sm:space-y-6 max-w-md md:max-w-lg lg:max-w-xl xl:max-w-2xl w-full"
...
<div className="relative bg-white">
  ...
  />
  <Label htmlFor="email" className="absolute left-12 top-2 font-semibold text-xs sm:text-sm text-[#6E6F72]">
    Email Address
  </Label>
  {errors.email && (
    <p className="mt-1 text-red-500 text-xs sm:text-sm absolute left-12 top-[100%]">
      Email is required
    </p>
  )}
</div>
    ...
    <p className="mt-1 text-red-500 text-xs sm:text-sm absolute left-12 top-[100%]">
      Password is required
          ...
          <div className="flex items-center justify-between">
            <label htmlFor="remember" className="flex items-center space-x-2 sm:space-x-3 cursor-pointer">
              <Switch id="remember" className="data-[state=checked]:bg-[#599400]" />
              <span className="text-xs sm:text-sm md:text-base text-[#6E6F72]">Remember Me</span>
            </label>
